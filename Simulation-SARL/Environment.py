...
class Environ:
    ...
    def step(self, action_power, action_phase):
        # 计算执行卸载和本地部分
        self.vehicle_rate = np.zeros(self.n_veh)
        ...
        self.Reward = np.mean(per_user_reward)
        return self.Reward, self.DataBuf, self.data_t, self.data_p, over_power, self.over_data
